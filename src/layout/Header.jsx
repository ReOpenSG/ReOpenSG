...
function Header() {
  ...
  const handleLanguageActive = (lang) => {
    ...
    setLanguage(lang);
    i18n.changeLanguage(lang === 'KOR' ? 'KOR' : 'ENG');
  };
  ...
  return (
    <header
      ...
      className={`${styles.header} ${isScrolled ? styles.scrolled : ''} ${
        whitePage ? styles.black : ''
      } ${isHeaderBlack ? styles.black : ''}`}
    >
      <h1 className={styles.headerTitle}>오픈에스지</h1>
      <nav className={styles.headerNavigation}>
        <ul className={styles.headerList}>
          <li>
            <Link className={styles.logoLink} to="/">
              <img className={styles.logoImage} src={opensgLogo} alt="오픈에스지" />
            </Link>
          </li>
          {!isSitemapOpen && (
            <li className={styles.menu} onMouseEnter={() => handleMenuActive('AboutUs')}>
              <button
                ...
                type="button"
                onClick={() => handleMenuActive('AboutUs')}
              >
                About Us
              </button>
              {activeMenu === 'AboutUs' && (
          ...
          {!isSitemapOpen && (
            <li className={styles.menu} onMouseEnter={() => handleMenuActive('Products')}>
              <button
                className={styles.menuLink}
                type="button"
                onClick={() => handleMenuActive('Products')}
              >
                Service & Solution
              </button>
              {activeMenu === 'Products' && (
                ...
                >
                  <ul className={`${styles.subMenu} ${styles.subMenuServiceAndSolution}`}>
                    <li>
                      ...
                      <ul className={styles.solutionsList}>
                        <li className={styles.solutionsListItem}>
                          <div className={styles.solutionsBlueItem}>Equipment Control System</div>
                          <span className={styles.solutionsGrayItem}>
                            스마트팩토리 장비 제어 시스템
                          </span>
                        ...
                        <li className={styles.solutionsListItem}>
                          <div className={styles.solutionsBlueItem}>Smart Tools</div>
                          <span className={styles.solutionsGrayItem}>스마트팩토리 시뮬레이터</span>
                          <ul className={styles.solutionsWhiteList}>
                            <MenuLink
                              linkName="Map Designer"
                              linkAddress="solutions/Map Designer"
                            />
                            <MenuLink
                        ...
                        <li className={styles.solutionsListItem}>
                          <div className="-text- text-open-font-medium -text--open-accent-accent-1">
                            Monitoring Solution
                          </div>
                          <span className={styles.solutionsGrayItem}>
                            스마트팩토리 모니터링 솔루션
                        ...
                        <li className={styles.solutionsListItem}>
                          <div className={styles.solutionsBlueItem}>Comm Driver</div>
                    ...
                    <li>
                      ...
                      <ul className={styles.solutionsWhiteList}>
                        <MenuLink
                          linkName="Microsoft Hololens 2"
                          linkAddress="/devices/Microsoft"
                        />
                      </ul>
                    </li>
                    <div className={styles.solutionsSeperator} />
                    <li>
                      <div className={styles.solutionsBlueItem}>Industries</div>
                      <ul className={styles.solutionsWhiteList}>
          ...
          {!isSitemapOpen && (
            <li className={styles.menu} onMouseEnter={() => handleMenuActive('Support')}>
              <button
                className={styles.menuLink}
                type="button"
                onClick={() => handleMenuActive('Support')}
              >
                Support
              </button>
              {activeMenu === 'Support' && (
      ...
      <ul className={styles.headerButtonList}>
        <li className={styles.languageButton}>
          <div className={styles.languageButtonBox}>
            <button
              className={activeLanguage !== 'ENG' ? '' : styles.languageInactive}
              type="button"
              onClick={() => handleLanguageActive('KOR')}
            >
              KOR
            </button>
            ...
            <button
              className={activeLanguage === 'ENG' ? '' : styles.languageInactive}
              type="button"
              onClick={() => handleLanguageActive('ENG')}
            >
              ENG
            </button>
          </div>
